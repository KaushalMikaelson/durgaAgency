...
export function matchTractor({
  ...
  // Return array with attached metadata properties so both recommendations[0] and recommendations.bestMatch work!
  const result = [...scoredTractors];
  result.recommendedHpRange = `${recommendedMinHp} - ${recommendedMaxHp} HP`;
  result.bestMatch = scoredTractors[0] || null;
  result.alternative = scoredTractors[1] || null;
  result.allRanked = scoredTractors;
